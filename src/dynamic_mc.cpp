...
template<typename reader>
void alignment_contexts<reader>::read_alignment_1_2(const pw_alignment & pw, size_t & numkeep) {
//	pw.print();
	std::string context;
	for(size_t i=level; i>0; --i) {
		size_t numk = i-1;
...
//	std::cout << "first al context is "<<int(context.at(0)) << " " << int(context.at(1)) << std::endl;
	...
	while(at_al_col < pw.alignment_length()) {
		...
		size_t len;
		next_modification_1_2(pw, at_al_col, modc, onc, len);
		if(size_t(modc) >= 5+ NUM_DELETE_DYN && size_t(modc)< 5 + NUM_DELETE_DYN + NUM_KEEP_DYN){
			numkeep += dynamic_mc_model::modification_length(modc);
		}
		// add current known base to context
		std::string newcontext = context;
		newcontext.append(1, onc);
...
template<typename reader>
void alignment_contexts<reader>::read_alignment_2_1(const pw_alignment & pw, size_t & numkeep) {
...
/*	if(p.alignment_length()==1){
	...
	size_t numkeep1 = 0;
	size_t numkeep2 = 0;
        ccost12.read_alignment_1_2(p,numkeep1);
        ccost21.read_alignment_2_1(p,numkeep2);
/*	if(p.alignment_length()==1){
	...
	if(numkeep1 != 0){
		m1=((costs12.cost_sum + alignment_base_cost.at(a1).at(a2))/(1111*numkeep1));
	}else{
		m1= costs12.cost_sum + alignment_base_cost.at(a1).at(a2);
	}
	if(numkeep2 != 0){
		m2=((costs21.cost_sum + alignment_base_cost.at(a2).at(a1))/(1111*numkeep2));
	}else{
		m2=costs21.cost_sum + alignment_base_cost.at(a2).at(a1);
	}
        mcost.at(0) = m1;
